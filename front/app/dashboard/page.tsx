...
export default function DashboardPage() {
  ...
  const selectedPeak = mendozaPeaks.find(peak => peak.id === selectedPeakId) || null
  const isPanelOpen = selectedPeakId !== null

  const handlePeakSelect = (peakId: number | null) => {
    setSelectedPeakId(peakId)
  }

  const handleClosePeakPanel = () => {
    setSelectedPeakId(null)
  }

  const handleStartClimb = (peakId: number) => {
    const peak = mendozaPeaks.find(p => p.id === peakId)
    if (peak) {
      // Navigate to climb page with peak context
      router.push(`/climb?peak=${peakId}&mountain=${encodeURIComponent(peak.name)}`)
    }
  }
  ...
  return (
    <div className="relative min-h-screen bg-gradient-to-br from-background via-muted/20 to-background">
      ...
      <PeakDetailsSidePanel
        peak={selectedPeak}
        isOpen={isPanelOpen}
        onClose={handleClosePeakPanel}
