...
export function useAutoMint() {
  ...
  const simulateVerification = async (duration: number = 3000): Promise<void> => {
    const steps = [
  ...
  const processCampaignVerification = useCallback(async (
    ...
    try {
      ...
      const photoPromises = verificationData.photos.map(file => fileToBase64(file));
      ...
      try {
        ...
        tokenId = await withTimeout(mintClimbingNFT(reservation.mountainId, proofURI), 30000);
        ...
        if (mintError.message.includes("timeout") || mintError.message.includes("Unexpected error")) {
          console.log("🔄 Using fallback demo mint mode");
          tokenId = `demo_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
          setCurrentStep("Usando modo demo debido a problemas de conexión...");
          await new Promise(resolve => setTimeout(resolve, 1000));
        } else {
          throw mintError; // Re-throw if it's not a timeout/connection error
        }
      }
  ...
  const processSummitVerification = useCallback(async (
    mountainId: number,
    mountainName: string,
    verificationData: VerificationData
  ): Promise<AutoMintResult> => {
    ...
    try {
      ...
      try {
        ...
        tokenId = await withTimeout(mintClimbingNFT(mountainId, proofURI), 30000);
        ...
        // Fallback to demo mode with mock token ID
        if (mintError.message.includes("timeout") || mintError.message.includes("Unexpected error")) {
          console.log("🔄 Using fallback demo mint mode for summit");
          tokenId = `summit_demo_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
          setCurrentStep("Usando modo demo debido a problemas de conexión...");
          await new Promise(resolve => setTimeout(resolve, 1000));
        } else {
          throw mintError; // Re-throw if it's not a timeout/connection error
      ...
      return {
        success: false,
        error: error.message || "Failed to complete summit verification and minting"
      };
    } finally {
      setIsProcessing(false);
      setVerificationProgress(0);
  ...
  const hasReservation = useCallback((campaignId: string): boolean => {
    if (!address) return false;
    return storageManager.hasReservation(campaignId, address);
  }, [address]);

  // Retry failed minting
  const retryMinting = useCallback(async (
    ...
    try {
      if (type === 'campaign') {
        ...
        storageManager.updateReservation(id, { status: 'minting', error: undefined });
        ...
        try {
          ...
          if (mintError.message.includes("timeout") || mintError.message.includes("Unexpected error")) {
            tokenId = `retry_demo_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
        ...
        // Update status and retry
        storageManager.updateSummitVerification(id, { status: 'minting', error: undefined });
        
        const proofURI = createMockProofURI(verification.mountainId);
        let tokenId: string | null = null;
        
        try {
          tokenId = await withTimeout(mintClimbingNFT(verification.mountainId, proofURI), 30000);
          ...
          // Fallback to demo mode for retry
          if (mintError.message.includes("timeout") || mintError.message.includes("Unexpected error")) {
            tokenId = `summit_retry_demo_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
      ...
      if (type === 'campaign') {
        storageManager.updateReservation(id, { status: 'failed', error: errorMsg });
      } else {
        storageManager.updateSummitVerification(id, { status: 'failed', error: errorMsg });
      }
