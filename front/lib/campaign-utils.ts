...
export const campaignCreationSchema = z
  .object({
    ...
    prizePoolLSK: z
      ...
      .max(10, "Prize pool cannot exceed 10 LSK"),
    mountainIds: z
      .array(z.number())
      .min(1, "Select at least one mountain")
      .max(10, "Cannot select more than 10 mountains"),
...
export const canUserJoinCampaign = (
  campaign: Campaign,
  userAddress: string
): boolean => {
...
export const getCampaignStatus = (
  campaign: Campaign
): "upcoming" | "active" | "completed" | "ended" => {
  const now = Math.floor(Date.now() / 1000);

  if (now < campaign.startDate) return "upcoming";
  if (now > campaign.endDate)
    return campaign.prizeDistributed ? "completed" : "ended";
...
export const joinCampaign = async (
  ...
  try {
    // Validate join data
    const validation = campaignJoinSchema.safeParse({
      campaignId,
      userAddress,
    });
    if (!validation.success) {
...
export const startCampaignClimb = async (
  campaignId: string,
  mountainId: number
): Promise<{ success: boolean; error?: string }> => {
  try {
...
export const calculateCampaignDifficulty = (
  mountainIds: number[]
...
export const calculateTotalCampaignRewards = (
  mountainIds: number[],
  prizePoolLSK: number
): number => {
  ...
  // Add LSK pool converted to HIKE tokens (simplified conversion)
  const lskToHikeMultiplier = 1000; // 1 LSK = 1000 HIKE tokens (for demo)
  const lskRewards = prizePoolLSK * lskToHikeMultiplier;

  return baseRewards + lskRewards;
};
  ...
  return {
    ...
    attributes: [
      ...
      { trait_type: "Mountains", value: campaignData.mountainIds.length },
      {
        ...
        value: calculateCampaignDifficulty(campaignData.mountainIds),
      },
      { trait_type: "Prize Pool", value: campaignData.prizePoolLSK + " LSK" },
